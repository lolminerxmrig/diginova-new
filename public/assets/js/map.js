...
var IndexAction = {
    init: function () {
        ...
        if (isModuleActive('new_profile_orders')) {
...
var OrderDetailsAction = {
    init: function () {
        ...
        if (isModuleActive('new_profile_orders')) {
            ...
            this.initShipmentStatusSwiper();
            this.initRequestInvoice();
        }

        if(isModuleActive('ship_by_seller_profile')) {
    ...
    initRepurchase: function () {
        $('.js-repurchase-order-item').on('click', function () {
    ...
    initShipmentStatusSwiper: function () {
        var ordersSwiper = new Swiper('.js-profile-order-steps', {
            slidesPerView: 4,
            spaceBetween: 0,
            navigation: {
                nextEl: '.c-profile-order-steps__button--next',
                prevEl: '.c-profile-order-steps__button--prev'
            }
        });

        if (!!ordersSwiper) {
            for (var i = 0; i < ordersSwiper.length; ++i) {
                ordersSwiper[i].slideTo($(ordersSwiper[i].$el).closest('.c-profile-order').find('.js-profile-order-steps div.is-active').index() - 5);
            }
        }
    },

    initRequestInvoice: function () {
        $('.js-invoice-seller-select').change(function () {
            if ($(this).val() > 0) {
                $('.js-invoice-request-btn').removeClass('is-inactive');
                $('.js-invoice-request-btn').prop('disabled', false);
            } else {
                $('.js-invoice-request-btn').addClass('is-inactive');
                $('.js-invoice-request-btn').prop('disabled', true);
            }
        });

        $('.js-invoice-request-form').submit(function (e) {
            e.preventDefault();
            ...
            Services.ajaxPOSTRequestJSON(
                ...
                function (response) {
                    $('.js-invoice-request-btn').addClass('is-inactive');
                    $('.js-invoice-request-btn').prop('disabled', true);
                    var selected = $('.js-ui-select.js-invoice-seller-select option:selected');
                    $('.js-seller-name').html(selected.text());
                    $('.js-invoice-request-message').removeClass('u-hidden');
                    selected.remove();

                    if ($('.js-ui-select.js-invoice-seller-select option').length <= 1) {
                        $('.js-ui-select.js-invoice-seller-select, .js-invoice-request-btn').addClass('disabled');
                    }

                    $('.js-ui-select.js-invoice-seller-select').selectric('refresh');
                },
                function (response) {
...
var UserHistory = {
    ...
    initRemoveHistoryItem: function () {
        $('.js-remove-item-profile-history').click(function () {
            ...
            DKConfirm(
                isModuleActive('new_user_history') ? 'آیا از حذف این کالا از لیست بازدیدهای اخیر اطمینان دارید؟' : 'آیا مطمئنید که این محصول از لیست بازدیدهای اخیر شما حذف شود؟',
                function () {
                    Services.ajaxPOSTRequestJSON(
        ...
        cardButton.on('click', function () {
            ...
            if(isModuleActive('iban')) {
                Services.ajaxPOSTRequestJSON(
                    ...
                    function (response) {
                      refundTableValue.text('حساب بانکی');
                      window.DKToast('حساب بانکی به عنوان روش بازگشت وجه انتخاب شد.');
    ...
    initSendConfirmCode: function () {
        ...
        $('.js-send-confirm-code').click(function (e) {
            ...
            window.Services.ajaxPOSTRequestJSON(
                'personal-info/send-confirm-code',
                {
