...
/*[PATH @digikala/supernova-digikala-marketplace/assets/local/js/controllers/registerController/contactDetailsAction.js]*/
const contactDetailsAction = {
    init: function () {
        if (isModuleActive('marketplace_registration_post_number_validation')) {
            this.initPostNumber()
        }
        ...
        this.initSelect();
        this.initWarehouseNotification();
        this.initFmsgTypeNotification();

        if (isModuleActive('marketplace_registration_persian_numbers')) {
            this.initInputsWithFaDigits();
            window.acceptPersianNationalId = true;
        }

        if (isModuleActive('marketplace_seller_registration_address')) {
            this.initCoordinatesControls();
        }
        if (isModuleActive('marketplace_seller_registration_birth_date_check')) {
            this.initCheckBirthDate();
        }
    ...
    initShabaMask: function () {
        const $input = $('.js-sheba-field');
        window
            .Inputmask('IR 99-9999-9999-9999-9999-9999-99', {
                placeholder: 'IR __-____-____-____-____-____-__',
            })
            .mask($input);
    },
    ...
    initTabs: function () {
        ...
        const $tabsContent = $('[data-role="tab-content"]');
        const $link = $('#c-online');

        $link.on('click', function (e) {
            const $this = $(this);
            const state = $('[data-role="tab"]:checked').val();
            const companyName = $('[name="register[company_name]"]').val();
            const firstName = $('[name="register[first_name]"]').val();
            const lastName = $('[name="register[last_name]"]').val();
            const url = $this.attr('href');

            if (state === 'business' && companyName.length > 0) {
                $this.attr('href', url + '?id=' + state + '&name=' + companyName);

                return;
            } else if (state === 'private' && firstName.length > 0 && lastName.length > 0) {
                $this.attr('href', url + '?id=' + state + '&name=' + firstName + '&last=' + lastName);

                return;
            }

            e.preventDefault();
            $('[name="register[agreement]"]').valid();
        });
            ...
            function triggerTabContent()
            {
                if ($tab.is(':checked')) {
                    $tabsContent.each(function () {
                        const $content = $(this);
                        const tabActiveClass = 'c-reg-form__tab--active';

                        if ($content.prop('id') === $tab.data('target')) {
                        ...
                        function disableTabElements(disable)
                        {
                            const $contentElements = $content.find(':input, select');

                            $contentElements.each(function (_, item) {
                                item.disabled = disable || (disable === undefined && true);
                            });
                        }
                    });
                }
    ...
    initBlurControls: function () {
        const $controls = $('[data-role="blurForm"]');

        $controls.each(function () {
            const $input = $(this);
            const blurClass = 'c-reg-form__row--blur';
            const $blurForm = $('[data-role="' + $input.data('target') + '"]');

            if ($input.val() === '1' && $input.is(':checked')) {
                disableBlurElements(false);
            } else if ($input.val() === '0' && $input.is(':checked')) {
                disableBlurElements(true);
            }

            $input.on('change', function checkDisabledRule()
            {
                const $this = $(this);
            ...
            function disableBlurElements(disabled)
            {
                const $blurElements = $blurForm.find(':input');

                $blurElements.each(function setDisabled(_, item)
                {
                    item.disabled = disabled;
                });

                disabled ? $blurForm.addClass(blurClass) : $blurForm.removeClass(blurClass);
            }
        });
    },

    initContactDetailsForm: function () {
        const $form = $('#details-form');
        const $submitShebaForm = $('.js-submit-form');
        const $rejectShebaForm = $('.js-cancel-sheba');
        const $shebaNotificationLoader = $('#sheba-notification .c-loading');
        const $econtractModal = $(document.querySelector('#new-electronic-contract'));
        const validator = $form.validate({
            groups: {
                dateOfExpire: 'register[birth_day] register[birth_month] register[birth_year]',
            },
            rules: {
                'register[business_name]': {
                    ...
                    validate_business_name_symbols: true,
                    // unique_name: true,
                },
                'register[phone]': {
                    required: true,
                ...
                'register[address]': {
                    ...
                    validate_persian_and_numbers_and_english_numbers: true,
                },
                'register[post_code]': {
                    required: true,
                    minlength: 10,
                    maxlength: 10,
                    validate_persian_and_english_digits: true,
                    validate_post_zeros: isModuleActive('marketplace_registration_post_number_validation')
                },
                'register[first_name]': {
                    required: true,
                    maxlength: 255,
                ...
                'register[national_identity_number]': {
                    required: true,
                    // digits: true,
                    minlength: 1,
                    maxlength: 20,
                ...
                'register[company_economic_number]': {
                    required: false,
                    digits: true,
                    minlength: 12,
                    maxlength: 12,
                    company_economic_number: false,
                },
                ...
                'register[company_authorized_representative]': {
                    required: true,
                    maxlength: 255,
                    validate_persian_textonly: true,
                },
                'register[warehouse_phone]': {
                    required: '[name="register[has_warehouse_address"][value="1"]:checked',
                    validate_persian_and_english_digits: true,
                    minlength: 8,
                    maxlength: 8,
                },
                ...
                'register[shaba_number]': {
                    required: true,
                    minlength: 33,
                    maxlength: 33,
                    // unique_sheba: true,
                },
                // 'register[description_of_products]': {
                //   required: true,
                //   minlength: 2,
                //   maxlength: 255,
                // },
                'register[main_supply_category_id]': {
                    required: true,
                },
                ...
                'register[agreement]': {
                    required: true,
                    validate_online_contract_fields: true,
                },
                'register[lat_and_long]': {
                    required: false
                }
            },
            messages: {
                ...
                'register[address]': {
                    ...
                    validate_address: 'فقط کاراکترهای فارسی، عدد مجاز است',
                ...
                'register[date_of_birth]': {
                    required: 'وارد نمودن تاریخ تولد اجباری است',
                    validate_persian_date: 'تاریخ تولد وارد شده معتبر نمی‌باشد',
                },
                'register[national_identity_number]': {
                ...
                'register[company_name]': {
                    ...
                    maxlength: 'نام شرکت می بایست حداقل 2 و حد اکثر 100 کاراکتر باشد',
                    validate_persian_and_numbers: 'فقط کاراکترهای فارسی، عدد مجاز است',
                },
                'register[company_economic_number]': {
                ...
                // 'register[description_of_products]': {
                //   required: __(
                //     'form.general.description_of_products.validation.empty'
                //   ),
                //   minlength: __(
                //     'form.general.description_of_products.validation.tooshort'
                //   ),
                //   maxlength: __(
                //     'form.general.description_of_products.validation.toolong'
                //   ),
                // },
                'register[main_supply_category_id]': {
                    required: 'لطفا نوع کالاهایی را که میخواهید در دیجیکالا به فروش برسانید را مشخص کنید',
                },
                'register[number_of_products]': {
                ...
                'register[birth_day]': {
                    required: 'انتخاب تاریخ اجباری است',
                },
                'register[birth_month]': {
                    required: 'انتخاب تاریخ اجباری است',
                },
        ...
        validator.showBackendErrors();
        /** @var popupIban.iban */
        /** @var popupIban.owner */
        /** @var popupIban.bank */
        const popupIban = window.popupIban;

        if (popupIban) {
            $('#sheba-response-api').text(popupIban.iban);
            $('#sheba-owner-response-api').text(popupIban.owner);
            $('#sheba-bank-response-api').text(popupIban.bank);
            $('[name="register[agreement]"]').prop('checked', true);
            $shebaNotificationLoader.removeClass('c-loading--hidden');
            UIkit.modal('#sheba-notification').show();

            setTimeout(function () {
                $shebaNotificationLoader.addClass('c-loading--hidden');
            }, 1000);
        }

        // $('#btnSubmit').on('click', submitForm);
        $submitShebaForm.on('click', approveShebaDetails);
        $rejectShebaForm.on('click', hideShebaApproveForm);
        ...
        // $('#sheba-response-api').text(response.iban);
        // $('#sheba-owner-response-api').text(response.owner);
        // $('#sheba-bank-response-api').text(response.bank);
        // $shebaNotificationLoader.removeClass('c-loading--hidden');
        // UIkit.modal('#sheba-notification').show();
        //
        // setTimeout(function () {
        ...
        function hideShebaApproveForm(e)
        {
            e.preventDefault();
            UIkit.modal('#sheba-notification').hide();
        }

        function approveShebaDetails(e)
        {
            e.preventDefault();
            UIkit.modal('#sheba-notification').hide();
            $form.attr('action', '/verify/iban/');
            $form.submit();
        }
        ...
        function approveEcontract()
        {
            ...
            Services.ajaxGETRequestHTML(
                '/ajax/registration/contract-content/',
                null,
                function (data) {
        ...
        $approveContractBtn.on('click', function (e) {
            e.preventDefault();
            formContractAgreement.value = 'confirm';

            const $econtractForm = this.closest('form');
        ...
        function rejectElectronicContract(e)
        {
            ...
            Services.commonModalNotification(
                ...
                'فروشنده عزیز، عدم توافق با مفاد قرارداد همکاری به منزله عدم تمایل شما به همکاری با دیجی‌کالا تلقی شده و فرایند ثبت نام متوقف خواهد شد. در صورت تمایل به لغو ثبت نام روی دکمه توقف فرآیند ثبت نام و در غیر اینصورت روی دکمه بازگشت به فرم قرارداد کلیک کنید.',
                false,
                [
                    {
    ...
    initFmsgTypeNotification: function () {
        const $fmcgTypeNotification = $(document.querySelector('#fmcg-type-notification'));
        const $categoryType = $(document.querySelector('.js-supply-category'));
        if (!($fmcgTypeNotification.length && $categoryType)) {
            return;
        }

        checkFmsgCategory();
        $categoryType.on('change', checkFmsgCategory);

        function checkFmsgCategory()
        {
            const isFmcgCategory = $categoryType.val() === '8895';
            $fmcgTypeNotification[isFmcgCategory ? 'removeClass' : 'addClass']('uk-hidden');
        }
    },

    initWarehouseNotification: function () {
        const $warehouseDetailsControl = $('[name="register[has_warehouse_address]"][value="0"]');

        $warehouseDetailsControl.on('change', function () {
            const $this = $(this);
            const $citySelect = $('#city-id');
            const isOriginalAddressInTehran = $citySelect.val() === '1698'; // check chosen address value

            if (!isOriginalAddressInTehran && $this.is(':checked')) {
                UIkit.modal('#warehouse-notification').show();

                $('#warehouse-notification').on({
                    'hide.uk.modal': function () {
                        $('[name="register[has_warehouse_address]"][value="1"]').click();
                    },
                });
            }
        });
    ...
    initCoordinatesControls: function () {
        const thiz = this;
        const mapPopup = document.querySelector('#coordinates-popup');
        let isFirstInit = false;

        thiz.isMapPopupOpened = false;
        thiz.confirmCoordinates = document.querySelector('.js-coordinates-confirm');

        $(document).on('mousedown', '.js-coordinates-btn-zone', function (e) {
            e.preventDefault();
        });
        $(document).on('focus mouseup', '.js-coordinates-btn-zone', showMapCoordinates);
        ...
        function showMapCoordinates(e)
        {
            ...
            if (thiz.isMapPopupOpened) {
                return;
            } else {
                thiz.isMapPopupOpened = true;
            }

            const zone = this;
            thiz.formCoordinatesZone = zone;
            thiz.coordinatesInput = zone.parentNode.querySelector('input[type=hidden]');
            thiz.coordinatesValue = thiz.coordinatesInput.value;
            let position;
            ...
            if (thiz.coordinatesValue) {
                const latAndLng = thiz.coordinatesValue.split(';');
                position = {
        ...
        thiz.confirmCoordinates.addEventListener('click', function (e) {
            ...
            thiz.coordinatesInput.value = thiz.coordinates;
            thiz.formCoordinatesZone.parentNode.querySelector('.js-coordinates-icon').classList.add('is-filled');
            thiz.formCoordinatesZone.value = 'ثبت شد';
            UIkit.modal('#coordinates-popup').hide();
            document.querySelector('#landing-phone').focus();
        });

        $(mapPopup).on('hide.uk.modal', function () {
            thiz.isMapPopupOpened = false;
        });
    },
    ...
    initCheckBirthDate() {
        ...
        var thisYear = parseInt(new persianDate().format('YYYY').toEnglishDigits());
        var $birthYearSelect = $('.js-birth-year-select');
        var $birthMonthSelect = $('.js-birth-month-select');
        var monthVal = $birthMonthSelect.val();
        var $errorElement = $('#dateOfExpire-error:visible')
        var errorText = $errorElement.text();
        var hasError = $errorElement.length > 0;

        $.each($('.js-birth-year-select option'), function (index, item) {
           if (thisYear - $(item).val() < 18) {
               $(item).remove()
           }
        });
        ...
        $birthYearSelect
            .on('change', function () {
                // eslint-disable-next-line no-undef
                var thisMonth = parseInt(new persianDate().format('MM').toEnglishDigits());
                var monthsArray = [];
                var monthsLength = 12;

                if (thisYear - $(this).val() === 18) {
                    monthsLength = thisMonth;
                }

                for (var i = monthsLength; i > 0; i--) {
                    monthsArray.push(
                        {id: i, text: Services.convertToFaDigit(i)}
                    );
                }
                $birthMonthSelect.empty().select2({
                   placeholder: 'ماه',
                   minimumResultsForSearch: Infinity,
                   language: Services.selectSearchLanguage,
                   data: monthsArray.sort(function (a, b) {
                       return a.id - b.id
                   })
                })
                    .data('select2')
                    .$dropdown.addClass('c-ui-select__dropdown');
                // set last filled value
                if (monthVal) {
                    $birthMonthSelect.val(monthVal);
                    $birthMonthSelect.trigger('change');
                }

                // show error if there is error on birth date
                if (hasError) {
                    $('#dateOfExpire-error').css('display', 'block').text(errorText);
                }
            });

            if ($birthYearSelect.val()) {
                $birthYearSelect.trigger('change')
            }
    },
    initPostNumber() {
        $.validator.addMethod('validate_post_zeros', function (value) {
            return value.toEnglishDigits() !== '0000000000';
        })
    }
};
