...
<div class="c-checkout-aside js-checkout-aside">
  <div class="c-checkout-bill">
    <ul class="c-checkout-bill__summary">
      ...
      @if (!is_null($voucher_varints_cost) && $voucher_varints_cost > 0)
        <li>
          <span class="c-checkout-bill__item-title">
              کد تخفیف
          </span>
          <span class="c-checkout-bill__price c-checkout-bill__price--discount">
              {{ persianNum(number_format(toman($voucher_varints_cost))) }}
            <span class="c-checkout-bill__currency"> تومان </span>
          </span>
        </li>
      @endif
      ...
      <li class="hidden js-voucher-code-discount">
        ...
        <span class="c-checkout-bill__price" data-cost="{{ $sum_sale_price - $sum_promotion_price - $voucher_varints_cost }}">
          @if (!is_null($voucher_varints_cost) && $voucher_varints_cost > 0)
            {{ persianNum(number_format(toman($sum_sale_price - $sum_promotion_price - $voucher_varints_cost))) }}
          @else
      ...
      <li class="c-checkout-bill__total-price">
        ...
        <?php
          if (!is_null($voucher_varints_cost) && $voucher_varints_cost > 0) {
            $final_sum_price = toman($sum_sale_price - $sum_promotion_price + $sum_shipping_cost - $voucher_varints_cost);
          }
          else {
            $final_sum_price = toman($sum_sale_price - $sum_promotion_price + $sum_shipping_cost);
          }
          $current_amount = $sum_sale_price - $sum_promotion_price + $sum_shipping_cost;
        ?>
        <span class="c-checkout-bill__total-price--amount" id="cartPayablePrice" data-current-amount="{{ $current_amount }}">
          <span class="js-price" data-current-amount="{{ $current_amount }}"> {{ persianNum(number_format($final_sum_price)) }} </span>
