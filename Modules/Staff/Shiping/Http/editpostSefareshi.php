...
namespace Modules\Staff\Shiping\Http;


final class postSefareshi {
    ...
    const TYPES = [self::TYPE_SEFARESHI] ;
    ...
    public function getPrice( $codRequest = false ){
        $bime = config("postSefareshi.bime") ;
        $tax  = config("postSefareshi.maliat") ;

        $inSideKarmozd = config("postSefareshi.inSideKarmozd") ;
        $outSideKarmozd = config("postSefareshi.outSideKarmozd") ;
        $haghSabt = config("postSefareshi.haghsabt") ;
        $getPart = $this->setPart() ;
        $cod = config("postSefareshi.cod") ;
        $cost = $this->cost ;
        $price = $this->setPrice() ;
    ...
    private function setPrice(){
        ...
        if(  in_array( $weightType , ["higher" , 5000] ) ){
            $tariff = config( sprintf("postSefareshi.%s.tariff" , $type ) ) ; // از تنظیمات تعرفه رو گرفتیم
