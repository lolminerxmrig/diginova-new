...
@section('head')
    <script src="{{ asset('seller/js/edit-comment-validation.js') }}"></script>
...
<?php
    $comment->name = persianNum($comment->name);
?>
            ...
            <div class="c-content-page c-content-page--plain c-grid__row">
                <div class="c-grid__col">
                    <div class="c-content-page__header">
                        <span class="c-content-page__header-action">  ویرایش دیدگاه{{ $comment->name }}  </span>
                        <span class="c-content-page__header-desc"> از این صفحه می توانید دیدگاه {{ $comment->name }} را ویرایش کنید</span>
                    </div>
                </div>
            </div>
                              ...
                              c-grid__col--sm-6">
                                                            ...
                                                            <div class="field-wrapper">
                                                                <input type="text"
                                                                       ...
                                                                       name="en_name"  value="{{ old('en_name')?? $comment->en_name }}">
                                                            </div>
                                                            <div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="field-wrapper field-wrapper--textarea">
                                                                <textarea name="description"
                                                                          placeholder="توضیحات دیدگاهباید بین ۷۰ تا ۱۰۰ کلمه درباره‌ی تاریخچه و محصولات دیدگاهباشد …"
                                                                          class="c-content-input__origin c-content-input__origin--textarea js-textarea-words"
                                                                          rows="" maxlength="500">{{ old('description')?? $comment->description }}</textarea>
...
{{--                                                                       data-status="0" {{ old('type') == 1 ? 'checked' : ($comment->type == 1 ? 'checked' : '')  }}>--}}
                                                            ...
                                                            <div class="c-grid__col">
                                                                <fieldset class="c-content-upload">
                                                                    ...
                                                                    <div id="image-container">
                                                                        ...
                                                                        <div id="imagesSection" class="c-content-upload__uploads js-uploaded-section imagesSection2 {{ ($comment->media)?: 'hide-div' }}"
                                                                        >
                                                                            <h3 class="product-form__section-title product-form__section-title--gap">
                                                                                تصویر بارگذاری شده
                                                                            </h3>
                                                                            ...
                                                                            <ul id="imagesContainer" class="c-content-upload__gallery-list js-uploaded-list js-sortable-list uk-sortable">
                                                                                <li class="c-content-upload__gallery-row js-uploads-row li-error" id="1dsWB">
                                                                                    <div class="c-content-upload__img-container">
                                                                                        ...
                                                                                        <img name="uploaded" id="preview_uploading" src="{{ env('APP_URL') . '/' . $comment->media()->first()->path . '/'.$comment->media()->first()->name }}"
                                                                                             data-id="{{ ($comment->media()->first()->id)? $comment->media()->first()->id : 0 }}" class="c-content-upload__img js-upload-thumb upload-image">
                                                                                        @else
...
@section('script')
<script src="{{ asset('staff/js/comments.js') }}"></script>
...
$(document).on("change", "#uploadImage", function () {
    ...
    $.ajax({
        url: '{{route('staff.comments.uploadUpdate')}}',
        cache: false,
        contentType: false,
        processData: false,
        data: form_data,
...
$('#comment_form').on('submit', function (e) {
    ...
    $.ajax({
        ...
        success: function () {
            window.location.href = "{{ route('staff.comments.index') }}";
        },
        error: function ()
        {
            //
