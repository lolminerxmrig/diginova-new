...
@section('head')
    <script src="{{ asset('seller/js/edit-landing-validation.js') }}"></script>
...
<?php
    $landing->name = persianNum($landing->name);
?>
            ...
            <div class="c-content-page c-content-page--plain c-grid__row">
                <div class="c-grid__col">
                    <div class="c-content-page__header">
                        <span class="c-content-page__header-action">  ویرایش برند {{ $landing->name }}  </span>
                        <span class="c-content-page__header-desc"> از این صفحه می توانید برند  {{ $landing->name }} را ویرایش کنید</span>
                    </div>
                </div>
            </div>
                                                        ...
                                                        c-grid__col--flex-initial c-grid__col--sm-6">
                                                            ...
                                                            <div class="field-wrapper">
                                                                <input type="text" class="c-content-input__origin js-attribute-old-value"
                                                                       name="name" value="{{ old('name')?? $landing->name }}">
                                                            </div>
                                                            <div>
                                                            </div>
                                                        </div>
                              ...
                              c-grid__col--sm-6">
                                                            ...
                                                            <div class="field-wrapper">
                                                                <input type="text"
                                                                       ...
                                                                       name="en_name"  value="{{ old('en_name')?? $landing->en_name }}">
                                                            </div>
                                                            <div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="field-wrapper field-wrapper--textarea">
                                                                <textarea name="description"
                                                                          ...
                                                                          rows="" maxlength="500">{{ old('description')?? $landing->description }}</textarea>
...
{{--                                                                       data-status="0" {{ old('type') == 1 ? 'checked' : ($landing->type == 1 ? 'checked' : '')  }}>--}}
                                                            ...
                                                            <div class="c-grid__col">
                                                                <fieldset class="c-content-upload">
                                                                    ...
                                                                    <div id="image-container">
                                                                        ...
                                                                        <div id="imagesSection" class="c-content-upload__uploads js-uploaded-section imagesSection2 {{ ($landing->media)?: 'hide-div' }}"
                                                                        >
                                                                            <h3 class="product-form__section-title product-form__section-title--gap">
                                                                                تصویر بارگذاری شده
                                                                            </h3>
                                                                            ...
                                                                            <ul id="imagesContainer" class="c-content-upload__gallery-list js-uploaded-list js-sortable-list uk-sortable">
                                                                                <li class="c-content-upload__gallery-row js-uploads-row li-error" id="1dsWB">
                                                                                    <div class="c-content-upload__img-container">
                                                                                        @if(count($landing->media))
                                                                                        <img name="uploaded" id="preview_uploading" src="{{ env('APP_URL') . '/' . $landing->media()->first()->path . '/'.$landing->media()->first()->name }}"
                                                                                             data-id="{{ ($landing->media()->first()->id)? $landing->media()->first()->id : 0 }}" class="c-content-upload__img js-upload-thumb upload-image">
                                                                                        @else
...
@section('script')
<script src="{{ asset('staff/js/landings.js') }}"></script>
...
$(document).on("change", "#uploadImage", function () {
    ...
    $.ajax({
        url: '{{route('staff.landings.uploadUpdate')}}',
        cache: false,
        contentType: false,
        processData: false,
        data: form_data,
...
$('#landing_form').on('submit', function (e) {
    ...
    $.ajax({
        method: "POST",
        url: '{{ route('staff.landings.update') }}',
        data: {
            name: name,
            en_name: en_name,
        ...
        success: function () {
            window.location.href = "{{ route('staff.landings.index') }}";
        },
        error: function ()
        {
            //
