                                        ...
                                        <div class="c-ui-form__row" data-select2-id="10">
                                            <div
                                                ...
                                                <select
                                                    ...
                                                    <option value="promotion_name" selected>نام برند</option>
                                                    <option value="promotion_category">گروه کالا</option>
                                                </select>
                                            </div>
                                            <div
                            ...
                            <div class="c-card">
                                <div class="c-card__wrapper">
                                    <div class="c-card__header c-card__header--table">
                                        <a href="{{ route('staff.promotions.create') }}" target="_blank">
                                            <div class="c-mega-campaigns__btns-green-plus uk-margin-remove">
                                                ایجاد برند جدید
                                            </div>
                                        </a>
                                        @if($trashed_promotions->count())
                                        <div class="c-grid__col c-grid__col--lg-4">
                                            <a href="{{ route('staff.promotions.trash') }}" class="c-ui-btn js-view-all-orders">مدیریت برند های حذف شده</a>
                                        </div>
                                        @endif
...
{{--                                        {{ $promotions->links('staffpromotion::layouts.pagination.pagination') }}--}}
                                        ...
                                        <div class="c-ui-paginator js-paginator" data-select2-id="16">
                                            <div class="c-ui-paginator__total" data-rows="۶">
                                                تعداد نتایج: <span name="total" data-id="{{ $promotions->total() }}">{{ persianNum($promotions->total()) }} مورد</span>
                                            </div>
                                    ...
                                    <div class="c-card__body c-ui-table__wrapper">
                                        <table class="c-ui-table js-search-table js-table-fixed-header c-join__table"
                                            ...
                                            <tr class="c-ui-table__row">
                                                ...
                                                <th class="c-ui-table__header"><span
                                                        class="table-header-searchable uk-text-nowrap table-header-searchable--desc">نام برند (Promotion)</span>
                                                </th>
                                                <th class="c-ui-table__header"><span
...
{{--                                                </th>--}}
                                            ...
                                            @foreach($promotions as $key => $promotion)
                                                <tr name="row" id="{{$promotion->id}}" class="c-ui-table__row c-ui-table__row--body c-join__table-row">
                                                    <td class="c-ui-table__cell">
                                                        <span class="c-wallet__body-card-row-item"> {{ persianNum($promotions->firstItem() + $key) }} </span>
                                                    </td>
                                                    <td class="c-ui-table__cell" style="min-width: 90px">
                                                            @if(count($promotion->media))
                                                            <img src="{{ env('APP_URL') . '/' . $promotion->media()->first()->path . '/'.$promotion->media()->first()->name }}" width="65" height="65">
                                                            @endif
                                                    </td>
...
{{--                                                    </td>--}}
                          ...
                          c-join__btn--secondary-greenish" href="{{ route('staff.promotions.edit', $promotion->en_name) }}">ویرایش</a>
                                    ...
                                    <div class="c-card__footer" style="width: auto;">
                                        <a href="{{ route('staff.promotions.create') }}">
                                            <div class="c-mega-campaigns__btns-green-plus uk-margin-remove">
                                                ایجاد برند جدید
                                            </div>
                                        </a>
                                                ...
                                                <img class="c-join__promotion-info-img" src="https://seller.digikala.com/static/files/ac21f0e6.svg" alt="Empty">
                                                 @if($trashed_promotions->count())
                                                    ...
                                                    <p class="c-join__promotion-info-statement">
                                                        شما تعداد {{ persianNum($trashed_promotions->count()) }} برند حذف شده در Trash دارید برای مدیریت و بازگردانی آنها بر روی دکمه زیر کلیک کنید
                                                    </p>
                                                    <a class="c-join__btn c-join__btn--info-box c-join__btn--secondary-greenish"
                                                       href="{{ route('staff.promotions.trash') }}">ورود به صفحه Trash</a>
                                                @else
                                                    ...
                                                    <a class="c-join__btn c-join__btn--info-box c-join__btn--secondary-greenish"
                                                       href="{{ route('staff.promotions.create') }}">ایجاد برند جدید</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
...
$(document).on('click', '.c-ui-paginator__control a', function(e){
    ...
    if (pageType == 'index'){
        var url = "{{route('staff.promotions.ajaxPagination')}}?page="+page;
    }
    else if (pageType == 'only_special') {
        var url = "{{route('staff.promotions.filterByType')}}?page="+page;
    }
    else if(pageType == 'promotionSearch') {
        var url = "{{route('staff.promotions.promotionSearch')}}?page="+page;
    }
    else if(pageType == 'promotionCatSearch') {
        var url = "{{route('staff.promotions.promotionCatSearch')}}?page="+page;
    }

    if(pageType == 'promotionCatSearch' || pageType == 'promotionSearch')
    {
...
$(".search_type").on('change', function (){
    ...
    $.ajax({
        type: 'post',
        url: '{{route('staff.promotions.filterByType')}}',
        data: {
...
$(document).on('click','.delete-btn' , function (){
    ...
    $.ajax({
        method: 'post',
        url: "{{route('staff.promotions.moveToTrash')}}",
        data: {
            'id': promotion_id,
        },
        success: function (result){
...
$("#search-btn").on('click', function (){
    ...
    if (searchGroup == 'promotion_name')
    {
        var url = "{{route('staff.promotions.promotionSearch')}}";
    }
    else if(searchGroup == 'promotion_category')
    {
        var url = "{{route('staff.promotions.promotionCatSearch')}}";
    }
...
$("#searchClear").on('click', function () {
    $.ajax({
        method: 'post',
        url: "{{route('staff.promotions.ajaxPagination')}}",
        success:function(result)
        {
            $(".js-table-container").html(result);
        }
