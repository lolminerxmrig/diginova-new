                                    ...
                                    <table class="c-ui-table  js-search-table js-table-fixed-header c-join__table"
                                        ...
                                        @if(isset($units) && !is_null($units))
                                            @foreach($units->unique() as $unit)
                                                <tr name="row db-row" id="item-{{$unit->id}}"
                                                    ...
                                                    <td class="c-ui-table__cell" style="min-width: 90px">
                                                        <input type="text" name="db_unit_name" value="{{ ($unit->name)? $unit->name : '' }}" class="c-content-input__origin js-unit-old-value attr_name">
                                                    </td>
                                                    <td class="c-ui-table__cell c-ui-table__cell--small-text" style="text-align: right; min-width: 200px;">
                                                        <select name="db_unit_type"
                                                    ...
                                                    <td class="c-ui-table__cell c-ui-table__cell-desc c-ui--pt-15 c-ui--pb-15">
                                                        <div class="uk-flex uk-flex-column">
                                                            @if($unit->type == 0)
                                                                <input type="text" class="c-content-input__origin db_unit_input_tag c-ui-input--deactive val_field" value="" disabled>
                                                            @else
                                                                <input name='drag-sort' value='[{"value":"foo", "editable":true, "code":"5"}, {"value":"bar"}]'>
                                                            @endif
                                                        </div>
                                                    </td>
...
$('#submit-form').on('click', function (e) {
    ...
    var db_unit_name = $("input[name='db_unit_name']").map(function(){return $(this).val();}).get();

    var db_unit_type = $("select[name='db_unit_type']").map(function(){return $(this).val();}).get();

    var db_unit_input_tag = $("input.db_unit_input_tag").map(function(){return $(this).val();}).get();

    var unit_ids = $("input[name='unit_id']").map(function(){return $(this).val();}).get();
    ...
    $.ajax({
        ...
        data: {
            ...
            db_unit_names: db_unit_name,
            db_unit_types: db_unit_type,
            db_unit_values: db_unit_input_tag,
            db_unit_ids: unit_ids,
        },
...
$(document).on('click', '.remove-btn', function () {
    ...
    $(document).on('click', '.yes', function (){
        // <input name="deleted-row" hidden>

        if ( $(".remove-btn").closest(".hide-tr").hasClass('db-row'))
        {
            console.log('db row deleted');
            $(".remove-btn").closest(".hide-tr").hide();
            $(".hide-tr").find("input[name='db--unitname']").val('deleted');
...
$(document).on('click', '.c-mega-campaigns__btns-green-plus', function () {
    ...
    var tr = '<tr name="row" id="item-new" class="c-ui-table__row c-ui-table__row--body c-join__table-row row"><td class="c-ui-table__cell" style="padding-right:0;padding-left:23px">' +
        ...
        '<td class="c-ui-table__cell c-ui-table__cell--small-text td-select" style="text-align:right; min-width: 200px;"><input name="row" class="position-row" value="new" hidden><select name="unit_type" class="uk-input uk-input--select js-select-origin attr_type select2-hidden-accessible" ' +
